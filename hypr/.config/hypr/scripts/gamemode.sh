...
if [ "$HYPRGAMEMODE" = "true" ] ; then
    ...
    hyprctl eval "hl.config({ decoration = { shadow = { enabled = false }, blur = { enabled = false }, rounding = 15 } })"
    hyprctl eval "hl.config({ general = { gaps_in = 3, gaps_out = 3, border_size = 0 } })"
    exit
fi
